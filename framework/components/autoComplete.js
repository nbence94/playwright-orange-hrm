...
export default class AutoComplete extends BasePage {
  ...
  async search(value) {
    ...
    const option = this.panel.getByRole('option', { name: value, timeout: 10000 }).first();
    await this.actions.click(option, { errorMessage: '❌ No user with this name found! (AutoComplete)' });
